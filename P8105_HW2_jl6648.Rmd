...
```{r}
baseline_df =
  read_csv("data_mci/MCI_baseline.csv", na = ".") |>                             #same as chunk above
  janitor::clean_names() |>
  mutate(
...
```{r}
amyloid_only =
  anti_join(amyloid_df, baseline_mci_df, by = "id")
```
...
Some participants appear in only the baseline or amyloid dataset based on the id variable, such as id 14 only appear in baseline dataset. After joining two dataset together, dataset contains `r nrow(participant_combine)` observations and `r names(participant_combine)` variables. Specificlly `r nrow(participant_combine)/5` number of participants appears in both dataset. 